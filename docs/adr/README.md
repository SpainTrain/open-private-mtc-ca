...
| [ADR-0008](0008-ratify-mpl-2-0-weak-file-scoped-copyleft-in-the-license-allow-list.md) | Ratify MPL-2.0 (weak file-scoped copyleft) in the license allow-list | Accepted | MPL-2.0's obligation attaches only to the MPL files themselves (which we consume unmodified), unlike the whole-program GPL/AGPL/SSPL copyleft §6 bans; ratified in the deny.toml allow-list so `ammonia` (admin-UI sanitizer) is unchanged (mtc-t92.2). |
| [ADR-0009](0009-s3-overwrite-of-an-ever-retained-key-is-backend-defined-not-a-conformance-invariant.md) | S3 Overwrite of an ever-retained key is backend-defined, not a conformance invariant | Accepted | The S3 backend permanently refuses `Overwrite` of a key that ever held Object Lock retention (safe strictness) while cloud-memory allows it; the divergence is backend-defined and deliberately NOT a conformance-suite invariant, since retained objects are append-only log content never legitimately overwritten (mtc-gyo.2). |
| [ADR-0010](0010-acme-nonce-anti-replay-rests-on-single-use-not-the-wall-clock-ttl.md) | ACME nonce anti-replay rests on single-use, not the wall-clock TTL | Accepted | Anti-replay rests on single-use — `NonceStore::consume` removes the nonce on first use (reuse → `badNonce`, RFC 8555 §6.5) — NOT on the wall-clock TTL, which only bounds retention of unused nonces; robust to clock jumps since a consumed nonce is already gone. acme-core already compliant (mtc-1hp.4). |
| [ADR-0011](0011-aws-sdk-tls-stack-upgrade-off-vulnerable-rustls-webpki-aws-lc-rs.md) | aws-sdk TLS stack upgrade off vulnerable rustls-webpki (aws-lc-rs) | Accepted | cloud-aws/dev-replicator: swap aws-sdk-s3/aws-sdk-dynamodb's misleadingly-named `rustls` feature (legacy hyper-0.14/rustls-0.21, vulnerable rustls-webpki 0.101.7, RUSTSEC-2026-0098/0099/0104) for `default-https-client` (hyper-1.x/rustls-0.23/aws-lc-rs); feature-flag-only fix, zero source changes, clears the advisory and a pre-existing duplicate; outside the §14.4 FIPS boundary (HSM-signing only). |
<!-- adr-index-end -->
