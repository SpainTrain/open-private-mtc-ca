# ADR-0011: aws-sdk TLS stack upgrade off vulnerable rustls-webpki (aws-lc-rs)
...
added package names from this change. **This analysis is confirmed by the
orchestrator**: §14.4's FIPS boundary covers the HSM signing / key-protection
path (`is_fips_validated()` reports the deployed HSM's status, unaffected here),
not network transport TLS, so the aws-sdk TLS-backend change is outside it — no
posture change, no exception needed. This moves the ADR to Accepted.
...
- The `deny.toml` `skip-tree` entries for `aws-sdk-s3`/`aws-sdk-dynamodb`
  remain in place (still required, per the empirical check in Decision)
